...
static LV2_Handle instantiateGate_gui(const _LV2UI_Descriptor *descriptor, const char *plugin_uri, const char *bundle_path, LV2UI_Write_Function write_function, LV2UI_Controller controller, LV2UI_Widget *widget, const LV2_Feature *const *features) {

//	Gtk::Main::init_gtkmm_internals();

    MainWindow* gui_data = new MainWindow();
    gui_data->controller = controller;
    gui_data->write_function = write_function;

        *widget = gui_data->window();
...
static void cleanupGate_gui(LV2UI_Handle instance) {
    delete static_cast<MainWindow*>(instance);
    //free(instance);
}
